...
def generate_coco_anns(root_path='/home/zcj/github/od_fovea_location/data/od', split = [2600, 2800]):
# 根路径，里面包含images(图片文件夹)，id.od.txt(bbox标注)，classes.txt(类别标签),以及annotations文件夹(如果没有则会自动创建，用于保存最后的json)
    ...
    for phase in phases:
        ...
        for i, cls in enumerate(classes, 1):
            dataset['categories'].append({'id': i, 'name': 'od', 'supercategory': 'mark'})
        # print(dataset)
        # 读取images文件夹的图片名称
        ...
        if not os.path.exists(folder):
          os.makedirs(folder)
        json_name = os.path.join(root_path, 'annotations/image_info_test2018_allfovea.json')
...
generate_coco_anns(root_path='/home/zcj/github/od_fovea_location/data/od', split=[2600,2800])
